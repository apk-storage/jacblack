...
///   1. снять эталон СТАРЫМ парсером (JACBLACK_UPDATE_GOLDEN=1) и закоммитить;
///   2. переписать парсер;
...
///   JACBLACK_UPDATE_GOLDEN=1 dotnet test --filter Golden
/// </summary>
public static class GoldenSnapshot
{
    ...
    static void Compare(string tracker, string caseName, string actual)
    {
        ...
        if (Environment.GetEnvironmentVariable("JACBLACK_UPDATE_GOLDEN") == "1")
        {
        ...
        Xunit.Assert.True(File.Exists(path),
            $"нет эталона {path} — снять: JACBLACK_UPDATE_GOLDEN=1 dotnet test --filter Golden");
