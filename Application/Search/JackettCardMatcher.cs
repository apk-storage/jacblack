                        ...
                        if ((_n != null && _n == name) || (_o != null && _o == originalname))
                        {
                            if (is_serial == 1)
                            {
                                ...
                                if (t.types.Contains("movie") || t.types.Contains("multfilm") || t.types.Contains("anime") || t.types.Contains("documovie"))
                                {
                                    ...
                                    //
                                    // Допуск пробовали вернуть 07.08.2026 ради
                                    // «Обсессии» (карточка 2026, раздачи 2025) —
                                    // и откатили: карточка стала отдавать НОЛЬ
                                    // вместо сорока двух. Расхождение года
                                    // лечится не здесь: живой поиск такие
                                    // раздачи приносит, а заслон карточки
                                    // пропускает их по совпавшему коду.
                                    if (year > 0)
                                    {
                                        if (t.relased == year)
