    ...
    <div class="w-full max-w-md rounded-2xl bg-[var(--jb-surface,#1b1b1f)] p-5 shadow-xl">
      ...
      <div v-else class="space-y-3">
        ...
        >
          ...
          <template v-else>
            Связь есть, код задан, сервер отвечает — но вашего телевизора среди
            устройств нет. Значит на нём либо не открыта Лампа, либо код в её
            настройках (Настройки → Терминал) не совпадает с указанным здесь.
          </template>
        </div>
        <template v-else>
          <!--
