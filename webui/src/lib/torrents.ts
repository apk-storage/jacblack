...
export type TorrentItem = {
  ...
  /**
   * Адреса всех копий одного файла, когда их больше одной.
   *
   * Копии на разных трекерах свёрнуты в одну строку — файл тот же самый, и
   * качаться будет он же. Ссылки остаются, чтобы можно было выбрать трекер.
   */
  sources?: string[] | null
