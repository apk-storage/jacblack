    ...
    public partial class FileDB
    {
        ...
        public static void updateFullDetails(TorrentDetails t)
        {
            ...
            if (streams != null)
            {
                ...
                {
                    // Раньше заголовок дорожки приводился к нижнему регистру внутри
                    // цикла по 900 студиям — то есть 900 раз на каждую дорожку.
                    string streamTitle = s.tags.title.ToLowerInvariant();
        ...
        /// <summary>
        /// Студии в нижнем регистре, посчитанные один раз. Короче пяти символов
        /// отсеяны здесь же — раньше эта проверка делалась на каждой итерации.
        /// Объявлено после allVoices: статические поля инициализируются по порядку.
        /// </summary>
        static readonly (string name, string lower)[] voicesLower =
            allVoices.Where(v => v != null && v.Length > 4)
                     .Select(v => (v, v.ToLowerInvariant()))
                     .ToArray();
