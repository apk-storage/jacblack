...
namespace JacBlack.Infrastructure.Networking
{
    ...
    public static class CloudflareClearance
    {
        ...
        /// <summary>
        /// Идёт ли текущий запрос в полосе обхода. Нужно не только браузеру:
        /// у обхода и у поиска разная цена ожидания. Человек ждёт ответа здесь
        /// и сейчас, поэтому быстрому пути в поиске дан короткий предел; обход
        /// же потерпит, и обрывать ему тяжёлые страницы — значит терять их.
        /// </summary>
        public static bool ЭтоОбход => _crawlLane.Value;
        ...
        ///
        /// В <paramref name="postData"/> лежит тело формы, если это POST;
        /// null означает обычный GET. Понадобилось 10.09.2026, когда kinozal
        /// ушёл под проверку: инфо-хеш там берётся POST-ом на
        /// get_srv_details.php, обход стоял только на GET, и новые раздачи
        /// молча перестали попадать в базу. Листинг при этом браузер получал
        /// исправно, обновления шли, и в журнале было ровное
